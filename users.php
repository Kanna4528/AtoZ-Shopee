...
$qer="select * from users where (uemail='$email' or uname='$email') and upass='$password' ";	
//$qes=mysqli_query($con,$qer);


if(mysqli_query($con,$qer))
{
        session_start();
        $qer="select uname,uid,ugroup from users where (uemail='$email' or uname='$email') and upass='$password' ";
$qes=mysqli_query($con,$qer);
